...
class qqFileUploader
{
     ...
     */
    function handleUpload($uploadDirectory, $s_size = '', $replaceOldFile = FALSE)
    {
        ...
        session_start();
        $filename = mt_rand(1000000, 9999999) . date("U") . mt_rand(10, 99); #20121120 將檔名重新命名 by frog
        ...
        if ($this->file->save($uploadDirectory . DIRECTORY_SEPARATOR . $filename . $ext))
        {
            ...
            { //判斷指定大小數量決定縮圖張數
                if ($x != "m")
                    resizeimage($uploadDirectory, $uploadDirectory . DIRECTORY_SEPARATOR . $filename . $ext, $s_size[$x]["w"], $s_size[$x]["h"], $this->uploadName, true, $x);
                else
                    $sFileName = resizeimage($uploadDirectory, $uploadDirectory . DIRECTORY_SEPARATOR . $filename . $ext, $s_size["m"]["w"], $s_size["m"]["h"], $this->uploadName);
